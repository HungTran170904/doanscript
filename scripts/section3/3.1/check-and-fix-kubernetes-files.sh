...
export_aks_config() {
    ...
    echo -e "${YELLOW}Please provide AKS cluster information:${NC}"
    ...
    if [ -z "$aks_resource_id" ]; then
        print_error "Failed to get AKS resource ID. Please verify cluster name and resource group."
        return 1
    fi
    
    # Export ARM template
    if az group export --resource-group "$resource_group" --resource-ids "$aks_resource_id" > "$arm_template_path" 2>/dev/null; then
        print_success "ARM template exported: $arm_template_path"
        return 0
    else
...
show_menu() {
    ...
    echo -e "${GREEN}2.${NC} Export AKS configuration"
    echo -e "${GREEN}3.${NC} Fix permissions (with backup option)"
...
run_fix_permissions() {
    ...
    read -p "Create backup before fixing? (y/N): " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
...
show_file_status() {
    ...
    # Get current pods (in case pods array is not in scope)
    local current_pods=($(get_pods))
    
    if [ ${#current_pods[@]} -eq 0 ]; then
        print_warning "No pods found for status check"
        return 1
    fi
